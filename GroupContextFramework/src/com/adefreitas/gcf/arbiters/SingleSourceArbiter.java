...
public class SingleSourceArbiter extends Arbiter
{

	public SingleSourceArbiter(int requestType) 
	{
		super(requestType);
	}
			...
			ArrayList<ContextCapability> receivedCapabilities) 
	{
		...
		{		
			//gcm.log("GCM-Arbiter [" + request.getContextType() + "]", "Looking at " + capability);
			if (capability.getContextType().equals(request.getContextType()))
			{
		...
		{		
			...
			//boolean    isSelf 			= capability.getDeviceID().equals(gcm.getDeviceID());
...
//			}
		...
		gcm.log("GCM-Request [" + request.getContextType() + "]", "Best Capability: " + bestCapability);
