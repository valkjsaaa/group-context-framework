...
{	
	...
	{		
		...
		this.getGroupContextManager().sendContext(this.getContextType(), this.getGroupContextManager().getDeviceID(), new String[0],
			...
			for (Integer lightID : lightIDs)
			{
				...
				try
				{
					...
					if (instruction.getParameters().length == 4)
					{
						...
						jsonFactory.put("on", Boolean.parseBoolean(instruction.getParameters()[0]));
						jsonFactory.put("bri", Integer.parseInt(instruction.getParameters()[1]));
						jsonFactory.put("sat", Integer.parseInt(instruction.getParameters()[2]));
						jsonFactory.put("hue", Integer.parseInt(instruction.getParameters()[3]));
						
						// Performs the Command
						new HTTPAsyncTask().execute(lightCommand, lightURL, jsonFactory.toString());
					}
					else if (instruction.getParameters().length == 3)
					{
						...
						String json = "{\"on\":true,\"xy\":[" + instruction.getParameters()[1] + ", " + instruction.getParameters()[2] + "]}";
		...
		else if (instruction.getCommand().equals(LIGHT_SPECTRUM_COMMAND))
		{
			for (Integer lightID : lightIDs)
			{
				...
				try
				{
					JSONObject jsonFactory = new JSONObject();
					jsonFactory.put("on", Boolean.parseBoolean(instruction.getParameters()[0]));
					jsonFactory.put("bri", (int)Double.parseDouble(instruction.getParameters()[1]));
					jsonFactory.put("sat", (int)Double.parseDouble(instruction.getParameters()[2]));
					jsonFactory.put("hue", (int)((1.0 - Math.random() * 0.2) * Double.parseDouble(instruction.getParameters()[3])));
