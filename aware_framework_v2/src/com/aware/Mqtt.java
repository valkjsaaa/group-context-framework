...
public class Mqtt extends Aware_Sensor implements MqttCallback {
	...
	public void onCreate() {
        ...
        try {
            MQTT_CLIENT = new MqttClient(MQTT_URL, ""+Aware.getSetting(getContentResolver(), Aware_Preferences.DEVICE_ID).hashCode(), MQTT_MESSAGES_PERSISTENCE);
	 ...
	 * @author denzil
	 */
	private static class MQTTAsync extends AsyncTask<MqttConnectOptions, Void, Void> {
        private MqttClient asyncMqtt = null;
	    
        @Override
	    protected void onPreExecute() {
	        super.onPreExecute();
	        asyncMqtt = MQTT_CLIENT;
	    }
	    
	    @Override
        protected Void doInBackground(MqttConnectOptions... params) {
            try {
                if( ! asyncMqtt.isConnected() ) asyncMqtt.connect(params[0]);
	...
	private static boolean publish(String topicName, byte[] payload) {
	    if( MQTT_CLIENT != null && MQTT_CLIENT.isConnected() ) {
            ...
            if(Aware.DEBUG ) Log.d(TAG,"Client is not connected... failed to publish");
        }
	    return false;
	}
	...
	private static boolean subscribe(String topicName) {
	    if( MQTT_CLIENT != null && MQTT_CLIENT.isConnected() ) {
            try {
                ...
                Log.d("Mqtt.java", "Subscribe:  " + topicName);
                return true;
            } catch (MqttSecurityException e) {
                if(Aware.DEBUG) Log.e(TAG, e.getMessage());
            } catch (MqttException e) {
	        ...
	        if(Aware.DEBUG ) Log.d(TAG,"Client is not connected... failed to subscribe");
	    }
	    return false;
	}
	...
	private static boolean unsubscribe(String topicName) {
	    if( MQTT_CLIENT != null && MQTT_CLIENT.isConnected() ) {
            ...
            if(Aware.DEBUG ) Log.d(TAG,"Client is not connected... failed to unsubscribe");
        }
	    return false;
	}
	
	/**
	 * If connection is lost, try connecting again. If it fails again, turns off the service.
	 * AWARE monitoring services will bring it back again in 5 minutes.
	 */
    @Override
    ...
    /**
     * New MQTT message has arrived
     * - Sends the message as a broadcast for extensability
     * - If the topic is on ESM, displays the ESM.
     */
    @Override
    public void messageArrived(String topic, MqttMessage message) throws Exception {
        ...
        if( topic.equals(Aware.getSetting(getContentResolver(), Aware_Preferences.DEVICE_ID).hashCode()+"/broadcasts") ) {
            Intent broadcast = new Intent(message.toString());
            sendBroadcast(broadcast);
        }
        
        if( topic.equals(Aware.getSetting(getContentResolver(), Aware_Preferences.DEVICE_ID).hashCode()+"/esm") ) {
        ...
        if( topic.equals(Aware.getSetting(getContentResolver(), Aware_Preferences.DEVICE_ID).hashCode()+"/configuration") ) {
    ...
    /**
     * When the message was delivered successfully on the server
     * Do nothing for now.
     */
	@Override
